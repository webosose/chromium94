...
WebMediaPlayerWebRTC::WebMediaPlayerWebRTC(
    ...
    StreamTextureFactoryCreateCB stream_texture_factory_create_cb,
  ...
  if (compositor_type_ != compositor_type) {
    ...
    if (has_first_frame_) {
      ...
      compositor_task_runner_->PostTask(
          FROM_HERE, base::BindOnce(&WebMediaPlayerWebRTC::OnTransformChanged,
                                    weak_ptr_this_, video_transformation_));
    }
  }
...
void WebMediaPlayerWebRTC::OnTransformChanged(
  ...
  if (!bridge_) {
    auto new_video_layer = cc::VideoLayer::Create(
        video_frame_provider_impl_.get(), video_transformation_);
...
void WebMediaPlayerWebRTC::OnResumed() {
  ...
  if (HasVideo()) {
    if (is_render_mode_texture())
      video_frame_provider_impl_->SetFrameType(
          VideoFrameProviderImpl::kTexture);
#if defined(NEVA_VIDEO_HOLE)
    else
      video_frame_provider_impl_->SetFrameType(VideoFrameProviderImpl::kHole);
#endif
...
void WebMediaPlayerWebRTC::CreateVideoWindow() {
  ...
  if (!video_frame_provider_impl_) {
    video_frame_provider_impl_ = std::make_unique<VideoFrameProviderImpl>(
        stream_texture_factory_create_cb_, compositor_task_runner_);
