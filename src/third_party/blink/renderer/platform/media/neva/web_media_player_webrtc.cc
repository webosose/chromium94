...
bool WebMediaPlayerWebRTC::HandleVideoFrame(
  ...
  if (software_fallback_callback_.is_null() &&
    ...
    if (pipeline_status_ == media::DECODER_ERROR_RESOURCE_IS_RELEASED ||
...
void WebMediaPlayerWebRTC::OnTransformChanged(
  ...
  WebMediaPlayerMS::OnTransformChanged(video_transformation_);
}
...
const scoped_refptr<media::VideoFrame>& input_frame) {
  ...
  media::VideoDecoderConfig video_config = GetVideoConfig(input_frame);
