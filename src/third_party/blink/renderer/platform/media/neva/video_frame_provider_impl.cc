...
void VideoFrameProviderImpl::CreateStreamTexture() {
  NEVA_DVLOGTF(1);
  NEVA_DCHECK(!stream_id_);
  NEVA_DCHECK(!texture_id_);

  if (!stream_texture_factory_create_cb_)
    return;
...
void VideoFrameProviderImpl::OnReleaseTexture(
    const scoped_refptr<StreamTextureFactoryInterface>& factories,
    GLuint texture_id,
    ...
    case FrameType::kTexture: {
      ...
      return media::VideoFrame::WrapNativeTextures(
          ...
          media::BindToCurrentLoop(base::BindRepeating(
              &VideoFrameProviderImpl::OnReleaseTexture,
