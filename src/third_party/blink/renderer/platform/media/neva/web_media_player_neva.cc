...
using blink::WebMediaPlayerParams;
...
void WebMediaPlayerNeva::SetVolume(double volume) {
  ...
  pending_request_.pending_volume_ = absl::nullopt;
  ...
  client_->DidMediaMetadataChange(
      HasAudio(), HasVideo(),
      media::DurationToMediaContentType(
...
void WebMediaPlayerNeva::OnTimeUpdate() {
  media_session::MediaPosition new_position(
      ...
      base::TimeDelta::FromSecondsD(CurrentTime()), IsEnded());
  ...
  if (client_) {
    client_->DidPlayerMediaPositionStateChange(
        playback_rate_, duration_, base::TimeDelta::FromSecondsD(CurrentTime()),
        IsEnded());
  }
}
...
WebMediaPlayer::SurfaceLayerMode WebMediaPlayerNeva::GetVideoSurfaceLayerMode()
    const {
  NEVA_DCHECK(main_task_runner_->BelongsToCurrentThread());
...
void WebMediaPlayerNeva::OnBufferingStateChanged(
  ...
  switch (buffering_state) {
    case media::BUFFERING_HAVE_NOTHING:
      interpolator_.StopInterpolating();
      UpdateReadyState(WebMediaPlayer::kReadyStateHaveCurrentData);
      break;
    case media::BUFFERING_HAVE_ENOUGH:
      if (is_playing_)
        interpolator_.StartInterpolating();
      UpdateReadyState(WebMediaPlayer::kReadyStateHaveEnoughData);
...
void WebMediaPlayerNeva::OnCustomMessage(
  ...
  switch (media_event_type) {
    ...
    case media::MediaEventType::kMediaEventBroadcastErrorMsg:
      converted_event_type =
          WebMediaPlayer::MediaEventType::kMediaEventBroadcastErrorMsg;
      break;
    case media::MediaEventType::kMediaEventDvrErrorMsg:
      converted_event_type =
          WebMediaPlayer::MediaEventType::kMediaEventDvrErrorMsg;
      break;
    ...
    case media::MediaEventType::kMediaEventPipelineStarted:
      converted_event_type =
          WebMediaPlayer::MediaEventType::kMediaEventPipelineStarted;
      break;
  }
