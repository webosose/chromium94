...
constexpr base::TimeDelta kRetryInterval = base::TimeDelta::FromSeconds(30);

#if defined(USE_WEBRISK_DATABASE)
const int kMaxDiffEntries = 0;
const int kMaxDatabaseEntries = 0;
constexpr base::TimeDelta kFetchingTimeout = base::TimeDelta::FromSeconds(3);
#endif
...
void WebRiskFetchHashes::RunFetchStatusCallback(
  ...
  if (fetch_status_callback_) {
    std::move(fetch_status_callback_).Run(status);
  }
}
