#!/bin/bash
#
# Run Mockserver for testing with wam_demo

MOCKPORT=${MOCKPORT:-8888}
WEBPORT=${WEBPORT:-8887}
SLEEPSTEP=${SLEEPSTEP:-.5}
BACKOFFTRIES=${BACKOFFTRIES:-10}
MOCKSERVER_FILE="mockserver-netty-3.10.4-jar-with-dependencies.jar"
...
# Function for creating expectations for all paths in dir ../tests and for
# the root path (http://localhost:$MOCKPORT/ opens folder view from the root)
CREATE_EXPECTATIONS()
{
...
java $logoff_flag $logoff_all_flag -jar $MOCKSERVER_FILE -serverPort $MOCKPORT &
MOCKSRVPID=$!
...
echo "MockServer (PID=$MOCKSRVPID) and WebServer (PID=$WEBSRVPID)"
