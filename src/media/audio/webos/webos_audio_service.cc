...
bool WebOSAudioService::GetDeviceList(
  ...
  auto devices_size = devices->GetList().size();
  auto devices_value =
  ...
  for (size_t i = 0; i < devices_size; ++i) {
    ...
    if (devices_value->GetDictionary(i, &device)) {
      auto connected = device->FindBoolPath(kConnectedState);
      if (connected && *connected) {
        std::string display_id;
        std::string* device_name = device->FindStringPath(kDeviceName);
        std::string* device_details = device->FindStringPath(kDeviceDetail);
        std::string* display = device->FindStringPath(kDisplay);
        if (display) {
