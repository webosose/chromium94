...
bool UMediaClientImpl::onSeekDone() {
  main_task_runner_->PostTask(
      FROM_HERE, base::BindOnce(&UMediaClientImpl::DispatchSeekDone,
      weak_ptr_));
  return true;
}
...
bool UMediaClientImpl::onCurrentTime(int64_t currentTime) {
  main_task_runner_->PostTask(
      FROM_HERE, base::BindOnce(&UMediaClientImpl::DispatchCurrentTime, weak_ptr_,
                                currentTime));
  return true;
}
...
bool UMediaClientImpl::onBufferRange(
  ...
  main_task_runner_->PostTask(
      FROM_HERE, base::BindOnce(&UMediaClientImpl::DispatchBufferRange, weak_ptr_,
                                bufferRange));
  return true;
}
...
bool UMediaClientImpl::onSourceInfo(
  ...
  main_task_runner_->PostTask(
      FROM_HERE,
      base::BindOnce(&UMediaClientImpl::DispatchSourceInfo, weak_ptr_, sourceInfo));
  return true;
}
...
bool UMediaClientImpl::onAudioInfo(const struct ums::audio_info_t& audioInfo) {
  main_task_runner_->PostTask(
      FROM_HERE,
      base::BindOnce(&UMediaClientImpl::DispatchAudioInfo, weak_ptr_, audioInfo));
  return true;
}
...
bool UMediaClientImpl::onVideoInfo(const struct ums::video_info_t& videoInfo) {
  main_task_runner_->PostTask(
      FROM_HERE,
      base::BindOnce(&UMediaClientImpl::DispatchVideoInfo, weak_ptr_, videoInfo));
  return true;
}
  ...
  if (sourceInfo.numPrograms > 0 && sourceInfo.programInfo.size()) {
    ...
    if (has_video_ && sourceInfo.programInfo[0].videoTrackInfo.size() &&
      ...
      gfx::Size natural_video_size =
          pixel_aspect_ratio_.GetNaturalSize(gfx::Rect(coded_video_size));
...
bool UMediaClientImpl::onVideoInfo(
  ...
  main_task_runner_->PostTask(
      FROM_HERE,
      base::BindOnce(&UMediaClientImpl::DispatchVideoInfo, weak_ptr_, video_info));
  return true;
}
...
void UMediaClientImpl::DispatchVideoInfo(
  ...
  pixel_aspect_ratio_ = ParsePixelAspectRatio(video_info.pixelAspectRatio);
  gfx::Size natural_video_size =
      pixel_aspect_ratio_.GetNaturalSize(gfx::Rect(coded_video_size));
