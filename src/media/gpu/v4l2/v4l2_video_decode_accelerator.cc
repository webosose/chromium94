...
#if defined(USE_NEVA_V4L2_CODEC)
      if (buffer) {
        bitstream_buffer_id_map_[buffer->timestamp()] = input_id;
      }
#endif
...
bool V4L2VideoDecodeAccelerator::DequeueOutputBuffer() {
  ...
  if (buf->GetPlaneBytesUsed(0) > 0) {
    ...
    base::TimeDelta timestamp =
        base::TimeDelta::FromSeconds(buf->GetTimeStamp().tv_sec) +
        base::TimeDelta::FromMicroseconds(buf->GetTimeStamp().tv_usec);
    ...
    int32_t bitstream_buffer_id = it->second;
